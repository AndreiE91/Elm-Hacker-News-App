...
formatDuration duration =
    let
        ...
        builderComponent n unit =
            if unit == 0 then
                ""
            else
                String.fromInt unit ++ " " ++ toPlural n unit ++ " "
        dayFrag = builderComponent "day" duration.days
        hourFrag = builderComponent "hour" duration.hours
        minuteFrag = builderComponent "minute" duration.minutes
        secondFrag = builderComponent "second" duration.seconds

        frags = List.filter ((<) "") [ dayFrag, hourFrag, minuteFrag, secondFrag ]
